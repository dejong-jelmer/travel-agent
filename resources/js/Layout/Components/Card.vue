<template>
    <div class="max-w-md h-auto mx-auto bg-white rounded-2xl shadow-md overflow-hidden">
      <!-- Foto bovenaan -->
      <div class="h-[55%] min-h-[250px] bg-cover bg-center" :style="`background-image: url(${imageUrl})`"></div>

      <!-- Onderkant wit met subtitel -->
      <div class="p-5 space-y-4 text-left">
        <h3 class="text-xl font-semibold text-gray-700">{{ country }}</h3>
        <h4 class="text-xl font-semibold text-gray-700">{{ title }}</h4>

        <!-- Twee kolommen, linker kolom met twee subkolommen -->
        <div class="grid grid-cols-2 gap-4">
          <div class="grid grid-cols-2 gap-4 text-left">
            <div>
                <p class="text-sm text-gray-500">Reisdagen</p>
                <p class="text-sm text-gray-500 font-bold">8 dagen</p>
            </div>
            <div>
                <p class="text-sm text-gray-500">Vervoer</p>
                <p class="text-sm text-gray-500 font-bold">
                    <!-- <?xml version="1.0" encoding="iso-8859-1"?> -->
                    <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                    <svg fill="#000000" height="15px" width="15px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        viewBox="0 0 512 512" xml:space="preserve">
                        <g>
                            <g>
                                <path d="M417.468,408.013C427.691,404.659,435.2,395.349,435.2,384V102.4C435.2,45.935,389.265,0,332.8,0H179.2
                                    C122.735,0,76.8,45.935,76.8,102.4V384c0,11.349,7.509,20.659,17.732,24.013L0,512h34.586l46.549-51.2h349.739l46.549,51.2H512
                                    L417.468,408.013z M102.4,128h140.8v76.8H102.4V128z M102.4,281.6h38.4c7.057,0,12.8,5.743,12.8,12.8s-5.743,12.8-12.8,12.8h-38.4
                                    V281.6z M104.405,435.2l23.27-25.6h256.649l23.27,25.6H104.405z M409.6,307.2h-38.4c-7.057,0-12.8-5.743-12.8-12.8
                                    s5.743-12.8,12.8-12.8h38.4V307.2z M409.6,256h-38.4c-21.205,0-38.4,17.195-38.4,38.4s17.195,38.4,38.4,38.4h38.4V384H102.4v-51.2
                                    h38.4c21.205,0,38.4-17.195,38.4-38.4S162.005,256,140.8,256h-38.4v-25.6h307.2V256z M409.6,204.8H268.8V128h140.8V204.8z
                                    M102.4,102.4c0-42.419,34.381-76.8,76.8-76.8h153.6c42.419,0,76.8,34.381,76.8,76.8H102.4z"/>
                            </g>
                        </g>
                    </svg>
                </p>
            </div>
          </div>
          <div class="flex justify-end">
            <button class="bg-blue-500 text-white py-2 px-4 rounded-lg">Bekijk</button>
          </div>
        </div>
      </div>
    </div>
  </template>

  <script>
  export default {
    data() {
      return {
        imageUrl: "https://picsum.photos/400/225",  // Voeg hier je eigen afbeelding toe
        country: "Frankrijk",
        title: "De prehistorische mens en de grotten in de valei van de Vézère"
      };
    },
  };
  </script>

  <style scoped>
  /* Specifieke styling kun je hier nog toevoegen */
  </style>
