                ...
                <div class="relative flex-1 px-4 py-6">
                    <nav class="space-y-6">
                        ...
                        <div v-for="group in menuGroups" :key="group.id" class="space-y-1">
                            ...
                            <!-- Group Items -->
                            <TransitionRoot :show="!collapsedSections[group.id]"
                                enter="transition ease-out duration-200" enter-from="opacity-0 -translate-y-1"
                                    ...
                                    <template v-for="(item, index) in group.items" :key="index">
                                        ...
                                        <div v-if="item.children" class="space-y-1">
                                            <button @click="toggleItem(item.id)"
                                                ...
                                                <!-- Chevron -->
                                                <ChevronDownIcon class="ml-2 h-4 w-4 transition-transform duration-200"
                                                    :class="{ 'transform rotate-180': !collapsedItems[item.id] }"
                                                    aria-hidden="true" />
                                            </button>

                                            <!-- Sub-items -->
                                            <TransitionRoot :show="!collapsedItems[item.id]"
                                                ...
                                                <div class="ml-6 space-y-1">
                                                    <Link v-for="(child, childIndex) in item.children" :key="childIndex"
                                                        :href="child.path"
                                                    ...
                                                    <!-- Icon -->
                                                    <component :is="child.icon" class="mr-3 h-4 w-4 flex-shrink-0"
                                                        :class="{
                                        ...
                                        <!-- Active Indicator -->
                                        <ChevronRightIcon v-if="isCurrentPage(item.path)"
                                            class="ml-2 h-4 w-4 text-brand-primary" aria-hidden="true" />
                                        </Link>
                                    </template>
                                </div>
                            </TransitionRoot>
                        ...
                        <div class="pointer-events-none fixed inset-y-0 left-0 flex max-w-[80%] pr-10 sm:pr-16">
                            <TransitionChild as="template"
                                ...
                                <DialogPanel class="pointer-events-auto relative w-screen max-w-sm sm:max-w-md">
                                    ...
                                    <!-- Menu content -->
                                    <div
                                        class="flex h-full flex-col overflow-y-auto bg-gradient-to-b from-brand-primary to-brand-primary shadow-2xl">
                                        <!-- Header -->
                                                    ...
                                                    <TransitionRoot :show="!collapsedSections[group.id]"
                                                        ...
                                                        <div class="space-y-1">
                                                            <template v-for="(item, index) in group.items" :key="index">
                                                                    ...
                                                                    <TransitionRoot :show="!collapsedItems[item.id]"
                                                                        ...
                                                                        <div class="ml-6 space-y-1">
                                                                            <Link
                                                                                ...
                                                                                :key="childIndex" :href="child.path"
                                                                                class="group flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200"
                                                                                :class="{
                                                                                    'bg-white text-brand-primary shadow-lg shadow-white/20': isCurrentPage(child.path),
                                                                ...
                                                                <!-- Icon -->
                                                                <component :is="item.icon"
                                                                    class="mr-3 h-5 w-5 flex-shrink-0" :class="{
                                                                        'text-brand-primary': isCurrentPage(item.path),
                                                                        'text-white/60 group-hover:text-white': !isCurrentPage(item.path)
                                                                    }" aria-hidden="true" />
