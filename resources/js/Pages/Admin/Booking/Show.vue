        ...
        <div class="max-w-wide mx-auto">
            <div class="grid grid-cols-1 laptop:grid-cols-3 gap-8">
                ...
                <div class="laptop:col-span-2 space-y-8">
                            ...
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{ $t('admin.booking.show.trip')
                                    ...
                                    <DefaultLink :href="route('admin.trips.show', booking.trip)"
                                        ...
                                        {{ booking.trip?.name }}
                                    - {{ booking.trip?.formatted_countries }}
                                    </DefaultLink>
                            </div>
                            <div>
                    ...
                    <section class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.contact_name') }}</label>
                                <p class="mt-1 text-gray-900">{{ booking.contact?.name }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.contact_email') }}</label>
                                <p class="mt-1 text-gray-900">{{ booking.contact?.email }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.contact_telephone') }}</label>
                                <p class="mt-1 text-gray-900">{{ booking.contact?.phone }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.contact_address') }}</label>
                    ...
                    <section class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                        <div class="border-b border-gray-200 bg-white px-6 py-4">
                            <h2 class="text-lg font-semibold text-gray-700">{{ $t('admin.booking.show.travelers') }}
                            </h2>
                            <p class="mt-1 text-sm text-gray-700/30">{{ $t('admin.booking.show.travelers_subheader') }}
                            </p>
                        </div>
                        <div class="p-6">
                            <div class="grid grid-cols-1 laptop:grid-cols-2 gap-6">
                                <div v-if="booking.adults?.length">
                                    <h3 class="text-sm font-medium text-gray-700 mb-3">{{ $t('booker.adults',
                                        booking.adults.length) }}</h3>
                                    <div class="space-y-2">
                                ...
                                <div v-if="booking.children?.length">
                                    <h3 class="text-sm font-medium text-gray-700 mb-3">{{ $t('booker.children',
                                        booking.children.length) }}</h3>
                                    <div class="space-y-2">
                ...
                <div class="laptop:col-start-3 space-y-8">
                    ...
                    <section class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                        <div class="border-b border-gray-200 bg-white px-6 py-4">
                            <h2 class="text-lg font-semibold text-gray-700">{{ $t('admin.booking.show.booking_status')
                                }}</h2>
                        ...
                        <div class="p-6 space-y-4">
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                <span class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.booking_status') }}</span>
                                <BookingStatusBadge :status="booking.status">{{ booking.status_label }}
                                </BookingStatusBadge>
                            </div>

                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                                <span class="text-sm font-medium text-gray-700">{{
                                    $t('admin.booking.show.payment_status') }}</span>
                                <PaymentStatusBadge :status="booking.payment_status">{{ booking.payment_status_label }}
                                </PaymentStatusBadge>
                            </div>
                        </div>
                    </section>
