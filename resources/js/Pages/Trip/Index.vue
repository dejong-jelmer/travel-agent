            ...
            <div class="max-w-screen-wide laptop:max-w-screen-desktop mx-auto px-4 tablet:px-6 laptop:px-8">
                ...
                <div class="flex flex-col laptop:flex-row gap-8 laptop:gap-12 items-start">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-brand-light mb-6 flex items-center gap-2">
                            ...
                            <span v-if="hasActiveFilters && !isFiltering">
                                ...
                                <button @click="clearAll"
                                    class="underline hover:text-brand-primary transition-colors duration-200">
                                    {{ $t('trips.clear_all') }}
                                </button>
                            </span>
                        </p>
                        ...
                        <!-- Empty state -->
                        <div v-else
                            class="text-center py-20 px-6 rounded-2xl border border-brand-primary/20 bg-brand-secondary">
                            <p class="text-xl font-poppins font-semibold text-brand-primary mb-2">
                                {{ $t('trips.empty_title') }}
                            </p>
                    ...
                    <aside class="w-full laptop:w-64 laptop:shrink-0 laptop:sticky laptop:top-28">
                        <div class="rounded-2xl border border-brand-primary/20 bg-white shadow-sm p-6 space-y-5">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <button v-if="hasActiveFilters" @click="clearAll"
                            ...
                            <div>
                                ...
                                <ul class="space-y-1">
                                    <li v-for="country in countries" :key="country.code">
                                        <label
                                            class="flex items-center gap-3 cursor-pointer group py-1.5 px-2 rounded-lg hover:bg-brand-secondary transition-colors duration-200">
                                            <input type="checkbox" :value="country.code" v-model="selectedCountries"
                                                class="w-4 h-4 rounded border-brand-light cursor-pointer accent-[#2d5f6e]" />
                                            <span
                                                class="flex-1 text-sm text-brand-text group-hover:text-brand-primary transition-colors duration-200">
                                                {{ country.name }}
                                            </span>
                                            <span
                                                class="text-xs font-medium text-white bg-brand-light rounded-full px-2 py-0.5 min-w-[1.5rem] text-center">
                            ...
                            <div v-if="allTransports.length">
                                ...
                                <ul class="space-y-1">
                                    <li v-for="mode in allTransports" :key="mode.value">
                                        <label
                                            class="flex items-center gap-3 cursor-pointer group py-1.5 px-2 rounded-lg hover:bg-brand-secondary transition-colors duration-200">
                                            <input type="checkbox" :value="mode.value" v-model="selectedTransports"
                                                class="w-4 h-4 rounded border-brand-light cursor-pointer accent-[#2d5f6e]" />
                            ...
                            <div v-if="minDurationBound !== maxDurationBound">
                                ...
                                <div class="flex items-center gap-2 mb-3">
                                    <div class="flex-1">
                                        ...
                                        <input type="number" v-model.number="durationMin" :min="minDurationBound"
                                            :max="durationMax"
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <input type="number" v-model.number="durationMax" :min="durationMin"
                                            :max="maxDurationBound"
                                            class="w-full text-sm border border-brand-primary/20 rounded-lg px-2 py-1.5 text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-primary" />
                                    </div>
                                    <span class="text-xs text-brand-light mt-4">{{ $t('trips.filter_days_unit')
                                        }}</span>
                                </div>

                                <!-- Dual slider -->
                                <DualRangeSlider :min="minDurationBound" :max="maxDurationBound"
                                    v-model:modelMin="durationMin" v-model:modelMax="durationMax" />
                            </div>
                            ...
                            <div v-if="minPriceBound !== maxPriceBound">
                                ...
                                <div class="flex items-center gap-2 mb-3">
                                    <div class="flex-1">
                                        ...
                                        <div class="relative">
                                            <span
                                                class="absolute left-2 top-1/2 -translate-y-1/2 text-sm text-brand-light">€</span>
                                            <input type="number" v-model.number="priceMin" :min="minPriceBound"
                                                :max="priceMax"
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="relative">
                                            <span
                                                class="absolute left-2 top-1/2 -translate-y-1/2 text-sm text-brand-light">€</span>
                                            <input type="number" v-model.number="priceMax" :min="priceMin"
                                                :max="maxPriceBound"
                                                class="w-full text-sm border border-brand-primary/20 rounded-lg pl-5 pr-2 py-1.5 text-brand-text focus:outline-none focus:ring-1 focus:ring-brand-primary" />
                                        </div>
                                    </div>
                                ...
                                <!-- Dual slider -->
                                <DualRangeSlider :min="minPriceBound" :max="maxPriceBound" :step="10"
                                    v-model:modelMin="priceMin" v-model:modelMax="priceMax" />
                            </div>
